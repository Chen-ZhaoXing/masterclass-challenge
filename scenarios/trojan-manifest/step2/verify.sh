...
if ! ensure_policy /var/kyverno-policies/require-http-probes.yaml require-probes; then
...
if ! gate_policy /tmp/kyverno-canary-step2.yaml "require-probes" 6; then
...
if [ $APPLY_OUT_EXIT -eq 0 ]; then
    ...
    if ! gate_policy /tmp/kyverno-canary-step2.yaml "require-probes" 3; then
        broadcast "⚠️  $(gate_reason "require-probes")"
        broadcast "   Nothing is wrong with your answer - wait a few seconds and click Check again."
        exit 1
    fi
