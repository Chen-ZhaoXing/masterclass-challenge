...
if ! ensure_policy /var/kyverno-policies/require-labels.yaml require-labels; then
    broadcast "⚠️  The policy could not be loaded, so your manifest was never actually checked:"
    broadcast "$POLICY_APPLY_ERR"
    exit 1
fi

# This canary satisfies every EARLIER policy and violates only require-labels,
# so a rejection naming require-labels can only have come from require-labels itself.
...
# Prove require-labels is enforcing RIGHT NOW, before grading anything.
if ! gate_policy /tmp/kyverno-canary-step3.yaml "require-labels" 6; then
    broadcast "⚠️  $(gate_reason "require-labels")"
...
if [ $APPLY_OUT_EXIT -eq 0 ]; then
    ...
    if ! gate_policy /tmp/kyverno-canary-step3.yaml "require-labels" 3; then
        broadcast "⚠️  $(gate_reason "require-labels")"
        broadcast "   Nothing is wrong with your answer - wait a few seconds and click Check again."
        exit 1
    fi
    ...
    if ! printf '%s' "$APPLY_OUT" | grep -q "require-labels"; then
