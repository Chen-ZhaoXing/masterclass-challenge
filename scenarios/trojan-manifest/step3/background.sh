...
# kyverno-resource-validating-webhook-cfg to include require-labels, so the first
...
for _ in $(seq 1 30); do
    CANARY_OUT=$(kubectl apply --dry-run=server -f /tmp/kyverno-canary-step3.yaml 2>&1) && { sleep 2; continue; }
    printf '%s' "$CANARY_OUT" | grep -q "require-labels" && break
    sleep 2
done
