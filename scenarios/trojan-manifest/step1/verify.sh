...
if ! ensure_policy /var/kyverno-policies/require-resource-limits.yaml require-resources; then
...
if ! gate_policy /tmp/kyverno-canary.yaml "require-resources" 6; then
...
if [ $APPLY_OUT_EXIT -eq 0 ]; then
    ...
    if ! gate_policy /tmp/kyverno-canary.yaml "require-resources" 3; then
        broadcast "⚠️  $(gate_reason "require-resources")"
        broadcast "   Nothing is wrong with your answer - wait a few seconds and click Check again."
        exit 1
    fi
