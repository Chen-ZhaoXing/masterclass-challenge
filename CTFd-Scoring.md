...
| Hint # | Cost | Hint Text |
|--------|------|-----------|
| 1 | **Free** | Step 1: `statefulset.yaml` has no storage in it at all - you are adding a `volumeClaimTemplates` section, not filling in blanks in an existing one. The StorageClass is called `local-path`, the access mode should be `ReadWriteOnce`, and the size is 5Gi. |
